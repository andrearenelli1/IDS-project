...
Unlike the detection threshold, the stop condition is geometry-based and
noise-independent:
\begin{equation}
...
$10\,$m in 3-D from the source.  At the nominal $\sigma = 10^{-6}$ this
corresponds to $\mathrm{SNR}_{\rm stop} = \tau_{\rm stop}/\hat{\sigma} = 10^{3}$.
...
\textsc{Stop}: strong-signal hovering; triggers partner selection and orbit
assignment.
\textsc{Support}: circular orbit (CW or CCW) around the Stop drone, providing
spatial diversity for DCGD. In
\textsc{Search} and \textsc{Support} state are, the reference points are updated on arrival ($\delta = 0.3\,$m);
during \textsc{Track} the ES reference is updated every step.

\paragraph{Lawnmower lane layout.}
After noise calibration the detection range $r_{\rm det}=(m/\tau_{\rm det})^{1/3}$
determines the lane spacing. The workspace of width $W$ is divided into
\begin{align}
  N_{\rm lanes} &= N_{\rm passes}\times N, \nonumber\\
  N_{\rm passes} &= \max\!\Bigl(1,\,
    \Bigl\lceil\tfrac{\lceil W/(2r_{\rm det})\rceil}{N}\Bigr\rceil\Bigr),
  \label{eq:lanes}
\end{align}
equally spaced lanes ($\Delta x = W/N_{\rm lanes}$), using the minimum
number of passes needed to guarantee $\Delta x \leq 2r_{\rm det}$, so that
every workspace point lies within detection range of at least one lane centre.
Drone $i$ is assigned lanes $i,\;i{+}N,\;i{+}2N,\ldots$ (stride-$N$
interleaving), reversing the along-$y$ direction at each transition.
Adjacent-lane assignment keeps neighbouring drones close together,
maximising the probability that the communication graph remains connected
...
\node[state, fill=blue!12, draw=blue!60!black]
  ...
  {\bfseries\sffamily SUPPORT\\[3pt]
   \footnotesize\sffamily circular orbit (CW/CCW)\\
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\columnwidth]{trajectories_top_view.png}
  \caption{Top-down view of drone trajectories (solid: true, dashed: IMDCL
           estimate).  The red star marks the victim position.}
  \label{fig:mpc_traj}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\columnwidth]{trajectories_3d_view.png}
...
Fig.~\ref{fig:dcgd_conv} reports the DCGD source estimation error
during the \textsc{Track} phase of the representative run ($N=4$).
The estimate error decreases as the three active drones accumulate
measurements around the source; the ATC Combine step accelerates
convergence relative to isolated local gradient descent, confirming
the benefit of the diffusion architecture.
...
\begin{figure}[t]
  ...
  \caption{Source estimation error $\|\bm{\theta}_i - \bm{\theta}^*\|$
           during the \textsc{Track} phase ($N=4$).
           Dashed: local gradient descent only; solid: DCGD with ATC Combine.}
  \label{fig:dcgd_conv}
\end{figure}
