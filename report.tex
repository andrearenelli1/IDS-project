...
\begin{figure}[t]
  ...
  \vspace{1.5cm}}}
  \caption{MPC trajectories (top) and per-component control inputs (bottom) for
           three drones.  $\times$ markers indicate waypoint targets; dashed red
           lines mark the per-component bound $\pm a_{\max} = \pm6\,$m/s$^2$.
           The constraint is a box constraint on each axis independently
           ($\|u\|_\infty \leq a_{\max}$), not on the Euclidean norm.}
  \label{fig:mpc_traj}
\end{figure}
